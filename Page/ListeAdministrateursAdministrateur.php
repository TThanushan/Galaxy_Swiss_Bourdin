...
<body>
	...
	<div class="container">
		...
		<!-- Hero Section -->
		<div class="jumbotron">
			<h1 class="display-4 text-center"> Liste des <i class="fas fa-user-secret mr-3"></i>Administrateurs </h1>

		<?php 
			$connexion=new PDO('mysql:host=localhost;dbname=gsbbdd;charset=utf8','root', '');

			$requeteCount = "select count('id') total from Administrateur ";
			
			$executionCount = $connexion->query($requeteCount);

			$count = $executionCount->fetch();

			echo '<div style="font-size: 250%; margin-right: 26%;" class=" text-center">('.$count['total'].') </div>';
		?>
		</div>
